...
const ROOT_SHA256 =
  "5559383DDD6666EB38FF746D592C5F6686B1C3340F4FDD584076D32E60B5E858";
const ICA_SHA256 =
  "00F1EE5D117141F1092AC350EFC937396AE8FEC8FEF1A4BC9581B1B1A12D4669";
...
Deno.test("root .cer serves the exact root DER as application/pkix-cert with CORS", async () => {
  ...
  assertEq(
    await sha256HexUpper(der),
    ROOT_SHA256,
    "served root DER fingerprint",
  );
});
...
Deno.test("root .pem serves the PEM", async () => {
  ...
  assert(
    (await res.text()).includes("-----BEGIN CERTIFICATE-----"),
    "pem body",
  );
});
...
Deno.test("ICA .cer serves the exact ICA DER", async () => {
  const res = await handleRequest(
    req("/realreel-claim-signing-ca.cer"),
    assets,
  );
  assertEq(res.status, 200, "status");
  assertEq(
...
Deno.test("index page publishes the fingerprint derived from the served root", async () => {
  ...
  assert(
    (await res.text()).includes(ROOT_SHA256),
    "derived fingerprint present on page",
  );
});

Deno.test("OPTIONS preflight returns 204 with CORS", async () => {
  const res = await handleRequest(
    req("/realreel-c2pa-root.cer", "OPTIONS"),
    assets,
  );
...
Deno.test("POST is rejected 405 with an Allow header", async () => {
  const res = await handleRequest(
    req("/realreel-c2pa-root.cer", "POST"),
    assets,
  );
