...
const sources = [
  ...
  [
    'Android',
    'android/src/main/java/expo/modules/photoattest/PhotoAttestModule.kt',
  ],
];
const labels = [
...
for (const [platform, rel] of sources) {
  ...
  for (const [name, label] of labels) {
    if (!source.includes(`"${label}"`)) {
      console.error(
        `✗ ${platform} (${rel}) does not contain the canonical ${name} "${label}" — update the native literal(s) in lockstep with @realreel/c2pa-trust-core.`,
      );
      ok = false;
    }
  }
  ...
  if (!createdMatch) {
    console.error(
      `✗ ${platform} (${rel}) has no created_assertion_labels in BUILDER_SETTINGS_JSON.`,
    );
    ok = false;
    continue;
  }
  const created = new Set(
    createdMatch[1].split(',').map((l) => l.trim().replace(/^"|"$/g, '')),
  );
  createdLabelLists.set(platform, [...created].sort().join(','));
  for (const [, label] of source.matchAll(AUTHORED_LABEL_RE)) {
    if (!created.has(label)) {
      console.error(
        `✗ ${platform} (${rel}) authors "${label}" but BUILDER_SETTINGS_JSON's created_assertion_labels does not list it — c2pa-rs would route it to gathered_assertions.`,
      );
      ok = false;
    }
  }
...
if (new Set(createdLabelLists.values()).size > 1) {
  console.error(
    `✗ created_assertion_labels differ between platforms: ${[...createdLabelLists].map(([p, l]) => `${p}=[${l}]`).join(' vs ')}`,
  );
  ok = false;
}
if (!ok) process.exit(1);
console.log(
  `✓ native assertion labels in lockstep with @realreel/c2pa-trust-core: ${labels.map(([, l]) => `"${l}"`).join(', ')}`,
);
console.log(
  `✓ every authored assertion label is in created_assertion_labels on both platforms`,
);
