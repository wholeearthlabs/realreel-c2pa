...
export class VerifyError extends Error {
   ...
   *  server.ts error handler. Conventions: 'play-integrity' (Android),
   *  'app-attest' (iOS) and 'ocsp' (a wrap-mode parent's revocation status).
   *  Cross-platform / generic errors omit category so platform-specific
   *  alerts don't fire on them. */
