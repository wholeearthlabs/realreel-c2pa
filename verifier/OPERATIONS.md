...
Cost: the verifier reads the asset offline first and, only for a wrap whose capture source declares responders and only once every structural gate has passed, reads it a second time with OCSP fetch on — one extra parse (10 to 40 ms) plus one responder round trip, about 1.5 s on a fresh instance and tens of milliseconds once the connection is warm. An uploaded store therefore never drives requests before it has proved to be a valid wrap, and native RealReel uploads never touch the network. Residual on the pinned engine (c2pa-rs 0.90.22): the response's signature is verified but the responder's certificate is not chained to a trust anchor, and the request travels over plain HTTP, so a party on the path between Cloud Run and the responder could forge a `good` answer. c2pa-rs 0.91 closes this (#2542, #2616); see `CRJSON_HARNESS.md` for why the engine waits on a matching `c2patool`.
